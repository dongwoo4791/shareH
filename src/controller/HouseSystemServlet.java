...
public class HouseSystemServlet extends HttpServlet {
	...
	private void doHandle(HttpServletRequest request, HttpServletResponse response)
		...
		if (action.equals("/home.do")) {
			...
			boolean flag = reservation_dao.getInstance().insert(new reservation() {
			});
			if (flag) {
				out.print("<script>alert('예약 성공');location.href='reservationForm.do';</script>");
			} else {
				out.print("<script>alert('예약 실패');location.href='reservationForm.do';</script>");
			}
